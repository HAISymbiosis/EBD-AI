...
def _look_attractors(states: list[np.array]) -> tuple[bool, list[np.array]]:
    ...
    Args:
        states: list of states visited in consecutive steps.

    Returns:
        (True, [state]) if the list is a fixed point, (False, []) otherwise.
    """
...
def _look_periods(states: list[np.array], min_period_len: int=2, max_period_len: int=None) -> list[np.array]:
    ...
    Args:
        states: list of states visited in consecutive steps.
        min_period_len: shortest period to consider.
        max_period_len: longest period to consider. Half the list length if None.

    Returns:
        the states of one period if found, None otherwise.
    """
...
def look_pattern_states(fcm: FuzzyCognitiveMap, sim_steps: int, pattern_len: int=50, max_period_size: int=50) -> list[np.array]:
    ...
    Args:
        fcm: FuzzyCognitiveMap. The FCM to look for the attractor states.
        sim_steps: int. The number of steps simulated before looking for a pattern.
        pattern_len: int. The number of steps recorded to look for a pattern.
        max_period_size: int. The longest limit cycle to look for.

    Returns:
        list of np.array. The states of the fixed point or limit cycle. None if none was found.
    """
...
def study_attractors_FCM(fcm: FuzzyCognitiveMap, max_steps: int, random_inits: int=10) -> dict[tuple, list[np.array]]:
    ...
    Args:
        fcm: FuzzyCognitiveMap. The FCM to look for the attractor states.
        max_steps: int. The number of steps simulated before looking for a pattern.
        random_inits: int. The number of random initial states to try.

    Returns:
        dict mapping each initial state (as a tuple) to its attractor states. None if none were found.
    """
    attractors = {}
...
def attractors_report(attractors: dict[tuple, list[np.array]]) -> tuple[dict, dict]:
    ...
    Args:
        attractors: dict. The attractors found, as returned by study_attractors_FCM.

    Returns:
        a dict with the frequency of each pattern ('Chaotic' when none was found)
...
class FuzzyCognitiveMap:
    ...
    def __init__(self, connections: Union[np.array, pd.DataFrame], threshold: float=0.5) -> None:
        ...
        Args:
            connections: np.array or pd.DataFrame. A square matrix with the connections between the concepts.
            threshold: float, optional. When simulating steps the state
