...
def generate_random_rules(nAnts: int, nRules: int, n_linguistic_variables: int, class_names: np.array):
    ...
    Args:
        nAnts: number of ants in the classifier.
        nRules: number of rules in the classifier.
        n_linguistic_variables: number of linguistic variables in the classifier.
        class_names: classes in the classifier.

    Returns:
        a list of tuples with the antecedents and consequents of the rules.
    """
    rules = []
...
def membership_randomrule(rule, X: np.array, fuzzy_variables: list[fs.fuzzyVariable]):
    ...
    Args:
        ...
        X: data to evaluate.
        fuzzy_variables: fuzzy variables of the classifier.

    Returns:
        the membership of the data to the rule.
    """
...
def generate_null_distribution(classifier, X: np.array, y: np.array, n_samples:int, nRules: int):
    ...
    Args:
        ...
        nRules: number of rules to bootstrap.

    Returns:
        a list with the cross entropy of the rules in the null distribution.
    """
    null_distribution = []
    nAnts = X.shape[1]
    class_names = np.unique(y)
...
def _aux_compute_rule_p_value(rule, X: np.array, y: np.array, fuzzy_variables: list[fs.fuzzyVariable], null_distribution: np.array):
    ...
    Args:
        ...
        y: labels to evaluate.
        fuzzy_variables: fuzzy variables of the classifier.
        null_distribution: null distribution of the classifier.

    Returns:
        the p-value of the rule.
    """
...
def compute_rule_p_value(classifier, X: np.array, y: np.array, nSamples: int=100, nRules: int=10):
    ...
    Returns:
        a list with the p-value of the rules.
    """
